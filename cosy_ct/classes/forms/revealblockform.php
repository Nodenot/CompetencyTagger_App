 ...
 */
class cosy_ct_revealblockform extends moodleform {
    public function definition() {
        global $CFG, $DB, $PAGE;
        // Dont forget the underscore!
        $mform = & $this->_form;

        $mform->updateAttributes(['id' => 'revealblockform']);
        ...
        // Setting url of src, this way we can access trough js code.
        $mform->setType('urlreveal', PARAM_INT);
        $mform->addElement('hidden','urlreveal', $this->_customdata['url_reveal'], array(
            'id' => 'id_urlreveal',
        ));
        ...
        $PAGE->requires->js_call_amd('block_cosy_ct/revealblockform_cosy_ct', 'init');
    }
}