 ...
 */
class cosy_ct_bookblockform extends moodleform {
    public function definition() {
        global $CFG, $DB, $PAGE;
        // Dont forget the underscore!
        $mform = & $this->_form;

        $mform->updateAttributes(['id' => 'bookblockform']);
        ...
        $mform->addElement('hidden','chapterid', $this->_customdata['chapterid'], array(
            'id' => 'id_chapterid',
        ));
        ...
        $anchors = $DB->get_records_sql("select * from mdl_block_cosy_ct_anchors where {$compare}",array('containerid' => $container->id.'#_#'.$this->_customdata['chapterid']));
