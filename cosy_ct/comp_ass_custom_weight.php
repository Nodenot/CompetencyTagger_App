...
require_once($CFG->dirroot . '/blocks/cosy_ct/classes/forms/weight_comp_form.php');
...
$urlpage = new moodle_url('/blocks/cosy_ct/comp_ass_custom_weight.php');
...
$PAGE->set_heading(get_string('compassweighttitlepage', 'block_cosy_ct'));
$PAGE->set_title(get_string('compassweighttitlepage', 'block_cosy_ct'));
...
$pagevisornode = $recobjeditnode->add(get_string('compassweighttitlepage', 'block_cosy_ct'), $urlpage);
$pagevisornode->make_active();

$weight_form = new cosy_ct_weight_comp_form(
    $urlpage, array(
        'recobj_id' => $recobjid,
        'compass_id' => $compid,
    )
...
$data = [
    ...
    'anchorid' => $anchorid,
    'recobjid' => $recobjid,
    'weight_form' => $weight_form->render(),
];

echo $OUTPUT->header();

echo $OUTPUT->render_from_template('block_cosy_ct/comp_ass_custom_weight', $data);
